...
public class WrogLaser extends Thread {
    private int x,y,w=50,h=50, kierX =1,kierY =1,velX=1,velY=1,hp,movingType=0, direction =0,
            angle=270,circleCenterX=C.FRAME_WIDTH/2-w/2,circleCenterY=C.FRAME_HEIGHT/2-100-h/2,radius=300,time=0;


    JPanel panel;
   Image imgWrog = new ImageIcon(getClass().getClassLoader().getResource("wrogLaserLeft.gif")).getImage();
    Image imgWrog2 = new ImageIcon(getClass().getClassLoader().getResource("wrogLaserRight.gif")).getImage();
    public WrogLaser(int x, int y, int w, int h, int direction, JPanel panel){
        this.x=x;
        this.y=y;
        this.w=w;
        this.h=h;
    ...
    public WrogLaser(int x, int y, int w, int h, int velX, int velY,int direction, int movingType, JPanel panel){
        this.x=x;
        this.y=y;
        this.w=w;
        this.h=h;
    ...
    public WrogLaser(int x, int y, int w, int h, int velX, int velY, int direction, int movingType, int centerX, int centerY, int radius, int hp, GamePanel gamePanel) {
        this.x=x;
        this.y=y;
        this.w=w;
        this.h=h;
    ...
    public void draw(Graphics2D g){
        if(imgWrog!=null && getDirection()==0)
        g.drawImage(imgWrog, this.getX(),this.getY(),this.getW(),this.getH(), null);
        else if (imgWrog!=null && getDirection()==1) g.drawImage(imgWrog2, this.getX(),this.getY(),this.getW(),this.getH(), null);
        }
